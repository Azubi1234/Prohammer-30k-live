...
CAT=Path("Legiones Astartes.cat"); OUT=Path("inspection-r37-word-bearers-structure.txt")
...
def profiles(e):
    ...
    return out
def row(e,prefix=""):
    return f"{prefix}{e.tag.split('}')[-1]} id={e.get('id')} name={e.get('name')} type={e.get('type')} default={e.get('defaultAmount')} hidden={e.get('hidden')} costs={costs(e)} cons={cons(e)} profiles={profiles(e)}"
lines=[f"CAT={r.get('revision')}"]
for root in r.findall(f"./{C('selectionEntries')}/{C('selectionEntry')}"):
    if (root.get("id") or "").startswith("r41-unit-xvii-"):
